...
def codes_to_compact(codes):
    ...
    # Группируем подряд идущие коды с одинаковым количеством цифр
    result_parts = []
    i = 0
    while i < len(codes):
        current_digits = digit_count_of_number(codes[i])

        # Собираем все подряд идущие коды с таким же количеством цифр
        group_codes = []
        while i < len(codes) and digit_count_of_number(codes[i]) == current_digits:
            group_codes.append(codes[i])
            i += 1

        # Определяем маркер: 1→A, 2→B, 3→C, 4→D, ..., 26→Z
        marker_index = current_digits - 1  # 0→A, 1→B, ...
        if marker_index > 25:
            # Если больше 26 цифр — используем Z как fallback
            marker = "Z"
        else:
            marker = chr(ord("A") + marker_index)

        # Форматируем числа с ведущими нулями до нужной ширины
...
def compact_to_codes(compact_str):
    ...
    while i < len(s):
        ...
        # Проверяем на маркер (A-Z или a-z)
        ch_upper = ch.upper()
        if "A" <= ch_upper <= "Z":
            current_width = ord(ch_upper) - ord("A") + 1  # A=1, B=2, ...
            i += 1
            continue
...
def convert_lzw_to_compact():
    """
    Читает LZW-коды из текстового поля (через пробел),
    конвертирует в компактный формат и заменяет содержимое поля.
    """
...
def convert_compact_to_lzw():
    """
    Читает компактную строку (без пробелов, с маркерами A/B/C/...),
    конвертирует обратно в LZW-коды через пробел и заменяет содержимое поля.
    """
    txt = text_widget.get("1.0", tk.END).strip()
...
def lzw_to_image_from_text():
    ...
    try:
        # Пробуем распарсить как стандартный формат (через пробел)
        codes_str = txt

        # Если строка не содержит пробелов — это компактный формат
...
def lzw_to_image_from_file():
    ...
    try:
        ...
        # Автоопределение формата: компактный или стандартный
        if " " not in codes_str_clean and any(
            ...
            # Стандартный формат (через пробел)
...
compact_label = tk.Label(row3, text="Компактный формат:", font=("Consolas", 10, "bold"))
...
hint = tk.Label(
    ...
    "LZW-режим: коды сжатия через пробел. Компактный: A=1-знач., B=2-знач., C=3-знач., ... без пробелов. "
    "При восстановлении формат определяется автоматически.",
    anchor="w",
)
